...
анти-форензика, смена локали, известные ioc. работает с любым клиентом, где
можно задать свой base url.
...
- **поведенческие правила** (`rules/rules.json`): re2-регэкспы по категориям
  ...
  гоняются по собранному вводу инструмента / аргументам вызова / тексту ответа.
